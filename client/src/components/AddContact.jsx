...
import { createChat } from "../redux/slices/createChatSlice";
import { getSingleUser } from "../redux/slices/usersSlice";

const AddContact = ({ onClose, open, setIsOpen }) => {
    const dispatch = useDispatch();

    const { data, status, error } = useSelector((state) => state.users);
    const userr = useSelector(state => state.user);
    const chat = useSelector((state) => state.createchat);
    ...
    const newContact = () => {
        dispatch(createChat(contactQuery));
    }
        ...
        <Dialog onClose={onClose} open={open}>
            <div className="px-6 py-6 min-w-80 sm:min-w-96">
                <div className="flex justify-between items-center mb-8">
                    <span className="font-semibold text-xl text-gray-900"> New Contact </span>
                ...
                <div className="mb-4">                    
                    <FormControl sx={style} size="small" fullWidth>
                        <InputLabel> Add Contact </InputLabel>
                            <OutlinedInput id="outlined-basic-2" label="Search user"
                            value={contactQuery}
                ...
                <button className="w-full mt-6 py-2.5  transition-all rounded-full bg-gradient-to-r  flex justify-center items-center text-gray-50 from-purple-700 to-blue-700 hover:opacity-90" onClick={newContact}>
                    {status == "loading" ? (                    
